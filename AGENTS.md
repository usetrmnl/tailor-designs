...
```
{WIDTH}x{HEIGHT}-{TYPE}-{NAME}-{CREDIT}.{EXT}
```
...
- `{CREDIT}` - Optional credit information (e.g., artist name, source)
...
- `800x480-splash-a_fishing_party_winslow_homer-clevelandart_org.png`

## Gallery Features
The gallery webpage provides:
- Filter by screen type (splash/loading/all)
- Search functionality
- Responsive grid layout
- Modern UI with Tailwind CSS styling
- Visual distinction between splash and loading screens
- Clickable thumbnails that open in a modal view
- Download buttons for each screen
- Density customization (high/low density layouts)
- Screen counts for total and by type
